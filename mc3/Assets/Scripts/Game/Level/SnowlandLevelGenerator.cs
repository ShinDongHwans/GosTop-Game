...
public class SnowlandLevelGenerator {
    ...
    public static Level generate(int width, int height, int upStairNum, int downStairNum, int portal = -1, int floor = 0) {
        ...
        for(int k = Random.Range(0, 4); --k >= 0;) {
        ...
        for(int t = Random.Range(2, 6); --t >= 0;) {
            int x = Random.Range(5, width - 6);
            int y = Random.Range(5, height - 6);
            int rx = Random.Range(5, 10);
            int ry = Random.Range(5, 10);
            int door = Random.Range(0, 120);
            for(int i = 0; i < 120; i++) {
                float angle = Mathf.Deg2Rad * ((float)i) / 120f * 360f;
                float s = Mathf.Sin(angle);
                float c = Mathf.Cos(angle);
                int dx = (int)(s * rx);
                int dy = (int)(c * ry);
                if(x + dx >= 2 && x + dx < width - 2 && y + dy >= 2 && y + dy < height - 2) {
                    if((i - door + 5) % 120 < 5 || i % 30 == 0) {
                        if(level.tiles[x + dx][y + dy].blocksMove) groundCount++;
                        dig(theme, level, x + dx, y + dy);
                    } else {
                        if(!level.tiles[x + dx][y + dy].blocksMove) groundCount--;
                        wall(theme, level, x + dx, y + dy);
                    }
                }
            }
        }
        ...
        npcList = new Tuple<int, float>[] {
            new Tuple<int, float>(NPCInfo.FindInfoByName("Something Blue"), 0.5f),
            new Tuple<int, float>(NPCInfo.FindInfoByName("Snow Pig"), 0.5f),
            new Tuple<int, float>(NPCInfo.FindInfoByName("Glacier"), 0.5f),
            new Tuple<int, float>(NPCInfo.FindInfoByName("Godurum"), 0.5f),
            new Tuple<int, float>(NPCInfo.FindInfoByName("Ice Dragon"), 0.5f),
            new Tuple<int, float>(NPCInfo.FindInfoByName("Ice Elemental"), 0.5f),
        };

        itemNumber = 10 + (width + height) / 8;
        itemList = new Tuple<int, float>[] {
            new Tuple<int, float>(ItemInfo.FindInfoById("O Hammar"), 0.02f),
            new Tuple<int, float>(ItemInfo.FindInfoById("Moon Trident"), Mathf.Max(floor - 2, 0) * 0.1f),
            new Tuple<int, float>(ItemInfo.FindInfoById("Scythe"), 0.005f),
            new Tuple<int, float>(ItemInfo.FindInfoById("Aqua Staff"), Mathf.Max(floor - 3, 0) * 0.2f),
            new Tuple<int, float>(ItemInfo.FindInfoById("Ninja Star"), 0.02f),
            new Tuple<int, float>(ItemInfo.FindInfoById("Teleport Wand"), 0.002f),
            new Tuple<int, float>(ItemInfo.FindInfoById("Brave Knight"), Mathf.Max(floor - 2, 0) * 0.1f),
            ...
            new Tuple<int, float>(ItemInfo.FindInfoById("Black Pearl Amulet"), 0.0001f),
